...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        print(f"Number of examples: {len(eval_dataset)}")
        ...
        print(f"\nDEBUG - Will write predictions to: {output_file}")
        print(f"DEBUG - Label map: {processor.get_labels()}")
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if args.output_mode == "sequencetagging":
                    ...
                    # Keep input_ids as tensor for batch_to_conll
                    ...
                    try:
                        ...
                        tokenizer.batch_to_conll(
                            input_ids=input_ids,  # Keep as tensor
                        ...
                        print(f"Error writing predictions: {str(e)}")
        ...
        # Compute metrics
        result = compute_metrics(eval_task, preds.flatten(), out_label_ids.flatten())
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    """
    Load and cache features for training or evaluation
    
    Args:
        args: arguments containing data_dir, local_rank, model_name_or_path, etc.
        task: task name (e.g., 'pico', 'neoplasm')
        tokenizer: tokenizer instance
        evaluate: boolean, True for evaluation mode, False for training mode
    
    Returns:
        dataset: torch Dataset containing the features
    """
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}_{}".format(
            ...
            str(task),
            datetime.now().strftime("%Y%m%d")  # Add date to cache file
        ),
    )
    ...
    try:
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            # Create features from dataset file
    ...
    try:
        ...
        # Debug information about first example
        if len(dataset) > 0:
            ...
            label_counts = np.bincount(np.concatenate(all_labels)[np.concatenate(all_labels) != -100])
